...
class RecipeRepository(private val db: RecipeDatabase) {
    ...
    suspend fun create(name: String, settings: RecipeSettings): String = db.withTransaction {
        settings.validate(); val clean = name.trim(); require(clean.isNotEmpty()) { "Name is required" }
        ...
        dao.insertRevision(RevisionEntity(UUID.randomUUID().toString(), id, 1, RecipeJson.settings(settings).toString(), now)); id
    }

    suspend fun revise(id: String, settings: RecipeSettings) = db.withTransaction {
        settings.validate(); val row = requireNotNull(dao.get(id)); val current = requireNotNull(row.revisions.maxByOrNull { it.number }); val now = System.currentTimeMillis()
    ...
    suspend fun rename(id: String, name: String) = db.withTransaction {
        val clean = name.trim(); require(clean.isNotEmpty()); val row = requireNotNull(dao.get(id)); val conflict = dao.byName(clean.lowercase())
    ...
    suspend fun archive(id: String, archived: Boolean) { val row = requireNotNull(dao.get(id)); dao.updateRecipe(row.recipe.copy(archived = archived, updatedAt = System.currentTimeMillis())) }
    suspend fun delete(id: String) { val row = requireNotNull(dao.get(id)); require(row.recipe.archived) { "Only archived recipes can be permanently deleted" }; dao.delete(id) }
    ...
    suspend fun importCurrent(envelope:RecipeEnvelope,conflict:ImportConflict):Int {
        ...
        envelope.recipes.forEach { incoming ->
            ...
            when {
                existing==null -> { create(incoming.name,incoming.settings);imported++ }
                conflict==ImportConflict.OVERWRITE -> { revise(existing.id,incoming.settings);imported++ }
                conflict==ImportConflict.RENAME -> {
                    var suffix=2;var name=incoming.name+" "+suffix
                    while(dao.byName(name.lowercase())!=null){suffix++;name=incoming.name+" "+suffix}
                    create(name,incoming.settings);imported++
                }
                else -> Unit
            }
        }
