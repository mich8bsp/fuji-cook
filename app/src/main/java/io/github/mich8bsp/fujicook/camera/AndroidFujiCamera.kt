...
class AndroidFujiCamera(private val manager:UsbManager,private val device:UsbDevice):Closeable {
    ...
    private fun transfer(bytes:ByteArray){var at=0;while(at<bytes.size){val n=minOf(16*1024,bytes.size-at);val sent=connection.bulkTransfer(output,bytes,at,n,30000);require(sent>0){"USB write failed on endpoint 0x"+output.address.toString(16)+" after "+at+"/"+bytes.size+" bytes"};at+=sent}}
    private fun receive(operation:String,timeoutMs:Int):PtpContainer {
        val first=ByteArray(16*1024);val got=connection.bulkTransfer(input,first,first.size,timeoutMs)
        ...
        while(at<size){val n=connection.bulkTransfer(input,all,at,minOf(16*1024,size-at),timeoutMs);require(n>0){operation+": USB read failed after "+at+"/"+size+" bytes"};at+=n}
